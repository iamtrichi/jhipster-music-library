...
export class AlbumService {
    ...
    create(album: IAlbum): Observable<EntityResponseType> {
        ...
        return this.http.post<IAlbum>(this.resourceUrl,
                 copy ,
                { observe: 'response' })
    ...
    update(album: IAlbum): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(album);
        return this.http.put<IAlbum>(this.resourceUrl,
                 copy ,
                { observe: 'response' })
        .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }
    ...
    protected convertDateFromClient(album: IAlbum): IAlbum {
        ...
        releaseDate: album.releaseDate?.isValid() ? album.releaseDate.format(DATE_FORMAT) : undefined,
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach ((album: IAlbum) => {
                album.releaseDate = album.releaseDate ? dayjs(album.releaseDate) : undefined;
            });
        }
        return res;
    }
